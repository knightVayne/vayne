...
- (NSArray *)demoArray {
    if (!_demoArray) {
        _demoArray = @[@"A",@"B",@"C",@"D",@"E",@"F",@"G",@"H",@"I",@"J",@"K",@"L",@"M",@"N"];
    }
    return _demoArray;
}
- (void)viewDidLoad {
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    self.detail = self.demoArray[indexPath.row];
    NSLog(@"%@",self.detail);
}

- (void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender {
    if ([segue.identifier isEqualToString:@"gotoDetail"]) {
        LPViewController *lpVC = segue.destinationViewController;
        lpVC.myDetail = self.detail;
        NSLog(@"==");
    }
}
